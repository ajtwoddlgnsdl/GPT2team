...
*   **클라이언트 측 JWT 생성**: 호감도 점수처럼 민감하지 않지만 위변조는 막아야 하는 데이터를 클라이언트에서 직접 JWT로 서명하여 보내는 것은 서버의 부담을 줄이는 흥미로운 접근 방식입니다. 이 토큰은 클라이언트 기기 시간 오차로 인한 만료 문제를 방지하기 위해 `expiresIn` 속성을 제거하고, `story_ticket`의 만료 시간에 의존하여 유효성이 검증됩니다.
