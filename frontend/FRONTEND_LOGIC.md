...
2.  `storyId`의 접두사(`intro_`, `MAIN_` 등)와 `heroineName`을 조합하여 `assets/scripts/...` 내의 정확한 JSON 파일 경로를 동적으로 생성합니다.
    *   `storyId`가 `intro_`로 시작하면 `intro1` 폴더를 사용합니다.
    *   `MAIN_`, `ENDING_` 등으로 시작하면 각각 `main`, `ending` 폴더를 사용합니다.
    *   `intro1` 외의 스토리에서는 `heroineName`을 기반으로 하위 폴더 경로를 추가로 조합합니다. (예: `assets/scripts/main/heroine_A/MAIN_day1_...json`)
...
*   **클라이언트 측 JWT 생성**: 호감도 점수처럼 민감하지 않지만 위변조는 막아야 하는 데이터를 클라이언트에서 직접 JWT로 서명하여 보내는 것은 서버의 부담을 줄이는 흥미로운 접근 방식입니다. 이 토큰은 클라이언트 기기 시간을 기준으로 2시간의 유효 기간을 가지며, 클라이언트-서버 시계 오차로 인해 서버에서 `ExpiredSignatureError`가 발생할 가능성이 있습니다.
