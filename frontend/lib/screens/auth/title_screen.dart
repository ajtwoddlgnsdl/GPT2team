          ...
          actions: [
            TextButton(
              onPressed: () async {
                final userId = await ApiClient().storage.read(key: 'user_id');
                if (userId == null) {
                  debugPrint("유저 ID가 없습니다. 게스트 로그인을 먼저 1회 해주세요.");
                  return;
                }

                int offlineDays = int.tryParse(_offlineDaysCtrl.text) ?? 0;
                int cheatHour = int.tryParse(_cheatHourCtrl.text) ?? 14;
                ...
                try {
                  ...
                  final loginRes = await ApiClient().dio.post(
                    ...
                    options: Options(
                      headers: {'admin-key': _adminKeyCtrl.text},
                    ),
                  );

                  if (loginRes.statusCode == 200 &&
                      loginRes.data['status'] == 'success') {
                    await ApiClient().storage.write(
                      key: 'access_token',
